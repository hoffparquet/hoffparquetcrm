...
export function clientRowToApi(row, notes = [], quotes = [], invoices = [], orderSheets = []) {
  return {
    id: row.id,
    name: row.name,
    ...
    orderSheets: orderSheets.map(orderSheetRowToApi),
  };
}
...
export function orderSheetRowToApi(row) {
  return {
    id: row.id,
    number: row.number,
    dateCreated: row.date_created,
...
export function productRowToApi(row) {
  return {
    id: row.id,
    name: row.name,
...
export function installationJobRowToApi(row) {
  return {
    id: row.id,
    clientId: row.client_id,
    projectName: row.project_name,
    jobDate: row.job_date,
    fitterName: row.fitter_name,
    items: row.items || [],
    notes: row.notes,
    createdAt: row.created_at,
  };
