...
export const api = {
  ...
  listInstallationJobs: () => fetch("/api/installation-jobs").then(handle),
  saveInstallationJob: (job) =>
    fetch("/api/installation-jobs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(job),
    }).then(handle),
  deleteInstallationJob: (id) => fetch(`/api/installation-jobs/${id}`, { method: "DELETE" }).then(handle),
